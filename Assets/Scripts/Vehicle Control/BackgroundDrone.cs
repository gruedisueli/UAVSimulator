...
public class BackgroundDrone : DroneBase
{
    ...
    protected override void Move()
    {
        ...
        {
            if ( wayPointsQueue.Count == 0 ) wayPointsQueue = new Queue<Vector3>(vcs.FindPath(transform.position, vcs.GetRandomPointXZ(transform.position.y), 5, 1 << 8 | 1 << 9 | 1 << 13 | 1 << 14)); //note use of bit-wise shift operators as a handy way to create a layer mask
